FBW.vertical.protections = {
    General = {
        ...
        AoA = {
            ...
            COMPUTE_SP = function (X)
                ...
                FBW.vertical.protections.General.AoA.AOA_V_SP = math.min(math.max(FBW.vertical.protections.General.AoA.AOA_V_SP, get(Vaprot_vsw_smooth)), get(Valpha_MAX_smooth))
                ...
                FBW.vertical.protections.General.AoA.AOA_SP   = math.min(math.max(FBW.vertical.protections.General.AoA.AOA_SP, get(Aprot_AoA)), get(Amax_AoA))
            end,
            ...
            Q = {
                LIM = function (X, Q, CLAMP_MARGIN, MIN_Q, MAX_Q, pid_array)
                    ...
                    local blend_ratio = Math_rescale(get(Aprot_AoA) - ENTRY_MARGIN, 0, get(Aprot_AoA), 1, FILTERED_AOA)
    ...
    ALT_prot = {
        ...
        VLS = function (X)
            ...
            local PROT_X = Math_rescale(get(VLS) - 15, DN_RATIO, get(VLS), X, FBW.filtered_sensors.IAS.filtered)
    ...
    Flight = {
        ...
        G_Pitch = function (G)
            ...
            return FBW.vertical.protections.General.Pitch.LIM_INPUT(G, INIT_MARGIN, UP_return_G, DN_return_G, FBW.vertical.dynamics.neutral_flight_G())
        end,

        HSP = {
            ACTIVE = false,
            PROT = function (G)
                local UP_return_G = FBW.vertical.dynamics.neutral_flight_G_NO_LIM() * 1.72
        ...
        AoA = function (G)
            local MAX_G_LIM = 2.5
            local MIN_G_LIM = FBW.vertical.dynamics.neutral_flight_G()
