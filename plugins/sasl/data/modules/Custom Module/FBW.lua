...
local A32nx_FBW_1G_command = {P_gain = 2.6, I_gain = 5.8, D_gain = 320, I_delay = 75, Integral = 0, Current_error = 0, Min_error = -22.5, Max_error = 22.5, Error_offset = 0}
...
function update()
    ...
    if get(G_load_command) == 1 then
        if get(FBW_status) == 2 then
            if get(FBW_pitch_mode) == 2 then--hold persec vpath
                ...
                set(Vpath_output, Set_anim_value(get(Vpath_output), Math_clamp(Math_clamp(FBW_PID(A32nx_FBW_1G_command, 1 - get(Total_vertical_g_load)), -1, get(AOA_lim)+1), get(Pitch_d_lim), get(Pitch_u_lim)), -1, 1, get(live_curve_speed)))
                --set(Vpath_output, Math_clamp(Math_clamp(FBW_PID_no_lim(A32nx_FBW_0G_command, 1 - get(Total_vertical_g_load)), -1, get(AOA_lim)+1), get(Pitch_d_lim), get(Pitch_u_lim)))
