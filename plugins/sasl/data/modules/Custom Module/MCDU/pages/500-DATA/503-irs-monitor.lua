...
function THIS_PAGE:L1(mcdu_data) mcdu_open_page(mcdu_data, 531) end

function THIS_PAGE:L2(mcdu_data) mcdu_open_page(mcdu_data, 532) end

function THIS_PAGE:L3(mcdu_data) mcdu_open_page(mcdu_data, 533) end

function THIS_PAGE:R5(mcdu_data)
    if adirs_how_many_irs_fully_work() == 3 then return end

    local input = mcdu_get_entry_simple(mcdu_data, {
        "###.#", "###", "##.#", "##", "#.#", "#"
    }, false)
    if input == nil then
