position = {1525, 1405, 160, 61}
size = {160, 61}

local overhead_integral = nil

local batLED = LED7Seg:new({}, "a321neo/led/bat1", 2, 1)
...
function draw()
    ...
    Draw_green_LED_num_and_letter(size[1] / 2 + 10, size[2] / 2 - 25, math.floor(math.abs(get(Elec_bat_1_V))), 2, 74, TEXT_ALIGN_RIGHT, 0.2, 1, 1, LED_cl, LED_cl_back)
    sasl.gl.drawText(Font_7_digits, size[1] / 2 + 16, size[2] / 2 - 25, ".", 74, false, false, TEXT_ALIGN_CENTER, LED_cl)
    Draw_green_LED_num_and_letter(size[1] / 2 + 40, size[2] / 2 - 25, Math_extract_decimal(get(Elec_bat_1_V), 1, true), 1, 74, TEXT_ALIGN_CENTER, 0.2, 1, 1, LED_cl, LED_cl_back)
    
    batLED:clear()
    batLED:display(get(Elec_bat_1_V))
end
