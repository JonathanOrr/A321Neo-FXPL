...
local function update_flaps_timer()
    if get(Slats) == 1 and get(Flaps_deployed_angle) == 30 then
        if RA_sys.flaps_full_time <= 10 then
...
local function RA_sensor_validation(sensors)
    for i = 1, #sensors do
        ...
        if adirs_get_avg_ias() > 200 and sensors[i].ALT_FT < 50 then
...
RA_sys.general_user = function ()
    ...
    local RA_ouptut = (RA_1 + RA_2) / 2

    --RA FAILURE or SHUNTDOWN
    if not RA_1_on and RA_2_on then
        return RA_1
    elseif RA_1_on and not RA_2_on then
        return RA_2
    elseif not RA_1_on and not RA_2_on then
        return 0
    end
...
RA_sys.single_RA_user = function (primary_RA)
    ...
    local RAs = {RA_sys.Sensors[1].ALT_FT, RA_sys.Sensors[2].ALT_FT}
    local RAs_on = {get(RA_sys.Sensors[1].Status_dataref) == 1, get(RA_sys.Sensors[2].Status_dataref) == 1}
