...
--image textures
local white_eye_img = sasl.gl.loadImage(moduleDirectory .. "/Custom Module/textures/white_eye.png")
local blue_eye_img = sasl.gl.loadImage(moduleDirectory .. "/Custom Module/textures/blue_eye.png")
...
local function resize_checklist(checklist_array)
    ...
    local window_height = 20 + 20 --20px for upper boarder and 20px for lower boarder

    for i = 1, #checklist_array do
        if checklist_array[i].Type == CHECKLIST_PROPERTIES then--if the item is a checklist property
            --sum 30px of box height to the window, but not need for the 10px of boarder(tree planting questions)
    ...
    for i = 1, #checklist_array do
        ...
        elseif checklist_array[i].Type == CHECKLIST_ITEM then--if the item is a checklist item
            ...
            if checklist_array[i].Viewing == false then--draw tick according to state
                sasl.gl.drawRectangle(390, rectangle_vertical_pos, 30, 30, LIGHT_GREY)--draw viewing click box
...
local function view_control(checklist_array)
    for i = 1, #checklist_array do
        ...
        elseif checklist_array[i].Type == CHECKLIST_ITEM then--if the item is a checklist item
            if checklist_array[i].Viewing == true then--viewing item
                if checklist_array[i].Quickview == VIEW_AVAILABLE then
                    set(Head_x, Set_anim_value(get(Head_x), checklist_array[i].Head_x, -1000000, 1000000, 5))
                    set(Head_y, Set_anim_value(get(Head_y), checklist_array[i].Head_y, -1000000, 1000000, 5))
                    set(Head_z, Set_anim_value(get(Head_z), checklist_array[i].Head_z, -1000000, 1000000, 5))
                    set(Head_phi, Set_anim_value(get(Head_phi), checklist_array[i].Head_phi, -1000000, 1000000, 5))
                    set(Head_psi, Set_anim_value(get(Head_psi), checklist_array[i].Head_psi, -1000000, 1000000, 5))
                    set(Head_the, Set_anim_value(get(Head_the), checklist_array[i].Head_the, -1000000, 1000000, 5))
                end
            end
        end
    end
...
function update()
    ...
    if head_returning == true then
        set(Head_x, Set_anim_value(get(Head_x), previous_head_x, -1000000, 1000000, 5))
        set(Head_y, Set_anim_value(get(Head_y), previous_head_y, -1000000, 1000000, 5))
        set(Head_z, Set_anim_value(get(Head_z), previous_head_z, -1000000, 1000000, 5))
        set(Head_phi, Set_anim_value(get(Head_phi), previous_head_phi, -1000000, 1000000, 5))
        set(Head_psi, Set_anim_value(get(Head_psi), previous_head_psi, -1000000, 1000000, 5))
        set(Head_the, Set_anim_value(get(Head_the), previous_head_the, -1000000, 1000000, 5))
