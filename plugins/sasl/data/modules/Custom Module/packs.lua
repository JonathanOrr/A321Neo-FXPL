...
function update()
    ...
    --HP valve logic--
    if get(Engine_1_avail) == 1 or get(Engine_mode_knob) == 1 or get(Engine_mode_knob) == -1 then
        set(L_HP_valve, 0)
    else
        set(L_HP_valve, 1)
    end

    if get(Engine_2_avail) == 1 or get(Engine_mode_knob) == 1 or get(Engine_mode_knob) == -1 then
        set(R_HP_valve, 0)
    else
        set(R_HP_valve, 1)
    end
    ...
    if get(Engine_1_avail) == 1 then
        set(ENG_1_bleed_switch, 1)
    else
        set(ENG_1_bleed_switch, 0)
    end

    if get(Engine_2_avail) == 1 then
        set(ENG_2_bleed_switch, 1)
    else
        set(ENG_2_bleed_switch, 0)
    end
    ...
    --bleed temp--
    if get(Left_bleed_avil) > 0.1 then
        if get(Left_bleed_avil) > 1.2 then--engine bleed
            set(L_bleed_press, Set_anim_value(get(L_bleed_press), 44, 0, 50, 0.35))
            set(L_bleed_temp, Set_anim_value(get(L_bleed_temp), 190, -100, 200, 0.35))
        else--apu bleed
            set(L_bleed_press, Set_anim_value(get(L_bleed_press), 39, 0, 50, 0.35))
            set(L_bleed_temp, Set_anim_value(get(L_bleed_temp), 150, -100, 200, 0.35))
        end
    else
        set(L_bleed_press, Set_anim_value(get(L_bleed_press), 0, 0, 50, 0.35))
        set(L_bleed_temp, Set_anim_value(get(L_bleed_temp), get(OTA), -100, 200, 0.35))
    end

    if get(Right_bleed_avil) > 0.1 then
        if get(Right_bleed_avil) > 1.2 then--engine bleed
            set(R_bleed_press, Set_anim_value(get(R_bleed_press), 44, 0, 50, 0.35))
            set(R_bleed_temp, Set_anim_value(get(R_bleed_temp), 190, -100, 200, 0.35))
        else--apu bleed
            set(R_bleed_press, Set_anim_value(get(R_bleed_press), 39, 0, 50, 0.35))
            set(R_bleed_temp, Set_anim_value(get(R_bleed_temp), 150, -100, 200, 0.35))
        end
    else
        set(R_bleed_press, Set_anim_value(get(R_bleed_press), 0, 0, 50, 0.35))
        set(R_bleed_temp, Set_anim_value(get(R_bleed_temp), get(OTA), -100, 200, 0.35))
    end
