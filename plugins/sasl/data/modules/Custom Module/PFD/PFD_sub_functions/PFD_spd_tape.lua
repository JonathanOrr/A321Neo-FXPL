...
local function draw_characteristics_spd(x, y, PFD_table)
    ...
    if not PFD_table.FAC_SPD_LIM_AVAIL() or (get(SFCC_1_status) == 0 and get(SFCC_2_status) == 0) then
        sasl.gl.drawText(Font_ECAMfont, x + size[1]/2-295, y + size[2]/2-200, "SPD", 42, false, false, TEXT_ALIGN_CENTER, ECAM_RED)
        sasl.gl.drawText(Font_ECAMfont, x + size[1]/2-295, y + size[2]/2-235, "LIM", 42, false, false, TEXT_ALIGN_CENTER, ECAM_RED)
    end
    ...
    if PFD_table.PFD_SPD_LIM_timer >= 0.7 then
        ...
        if get(FBW_total_control_law) == FBW_NORMAL_LAW then
            ...
            sasl.gl.drawText(Font_ECAMfont, x + size[1]/2-352, y + size[2]/2-16 + Math_rescale_no_lim(-43, -240, 42, 240, get(PFD_table.Vmax_prot_spd) - adirs_get_ias(PFD_table.Screen_ID)), "x", 34, false, false, TEXT_ALIGN_CENTER, ECAM_ORANGE)
        end
        ...
        if get(SFCC_1_status) == 1 or get(SFCC_2_status) == 1 then
            if get(Flaps_handle_position) == 1 then
                ...
                sasl.gl.drawText(Font_ECAMfont, x + size[1]/2-300, y + size[2]/2-22 + Math_rescale_no_lim(-43, -240, 42, 240, get(PFD_table.S_spd) - adirs_get_ias(PFD_table.Screen_ID)), "S", 42, false, false, TEXT_ALIGN_CENTER, ECAM_GREEN)
            end
            if get(Flaps_handle_position) == 2 or get(Flaps_handle_position) == 3 then
                ...
                sasl.gl.drawText(Font_ECAMfont, x + size[1]/2-300, y + size[2]/2-22 + Math_rescale_no_lim(-43, -240, 42, 240, get(PFD_table.F_spd) - adirs_get_ias(PFD_table.Screen_ID)), "F", 42, false, false, TEXT_ALIGN_CENTER, ECAM_GREEN)
            end
        end
...
local function draw_BUSS(x, y, PFD_table)
    ...
    sasl.gl.drawText(Font_ECAMfont, x + size[1]/2-352.5, y + size[2]/2-244 + PFD_table.BUSS_vsw_pos + 60 + 53 + 60 + 30, "FAST", 28, false, false, TEXT_ALIGN_CENTER, ECAM_WHITE)
    ...
    sasl.gl.drawText(Font_ECAMfont, x + size[1]/2-352.5, y + size[2]/2-244 + PFD_table.BUSS_vsw_pos - 20 - 30, "SLOW", 28, false, false, TEXT_ALIGN_CENTER, ECAM_WHITE)
    sasl.gl.resetClipArea ()
...
local function draw_decel_info(x, y, PFD_table)
    if get(Wheel_autobrake_is_in_decel) == 1 then
        sasl.gl.drawText(Font_ECAMfont, x + size[1]/2-387, y + size[2]/2-278, "DECEL", 35, false, false, TEXT_ALIGN_CENTER, ECAM_GREEN)
    end
end
...
local function draw_mach_info(x, y, PFD_table)
    if adirs_is_mach_ok(PFD_table.Screen_ID) == true then
        ...
        if PFD_table.Show_mach then
            sasl.gl.drawText(Font_ECAMfont, x + size[1]/2-387, y + size[2]/2-308, "." .. Round(adirs_get_mach(PFD_table.Screen_ID) % 1 * 1000), 35, false, false, TEXT_ALIGN_CENTER, ECAM_GREEN)
        end
    else
        sasl.gl.drawText(Font_ECAMfont, x + size[1]/2-387, y + size[2]/2-308, "MACH", 35, false, false, TEXT_ALIGN_CENTER, ECAM_RED)
    end
end

local function draw_airspeed_numbers(string, x, y)
    for i=1, 3 do
        sasl.gl.drawText(Font_ECAMfont, x + size[1]/2-420 + (i-1) * 21, y, string.sub(string, i, i) , 38, false, false, TEXT_ALIGN_CENTER, ECAM_WHITE)
    end
end
...
function PFD_draw_spd_tape(x, y, PFD_table)
    ...
    if adirs_is_ias_ok(PFD_table.Screen_ID) == false and not adirs_is_buss_visible(PFD_table.Screen_ID) then
        ...
        if PFD_table.SPD_blink_now == true then
            sasl.gl.drawText(Font_ECAMfont, x + size[1]/2-390, y + size[2]/2-20, "SPD", 42, false, false, TEXT_ALIGN_CENTER, ECAM_RED)
        end
    end
