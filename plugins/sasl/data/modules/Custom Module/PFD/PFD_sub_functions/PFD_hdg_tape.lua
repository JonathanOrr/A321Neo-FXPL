local function draw_hdg_tape_symbols(PFD_table)
    SASL_draw_img_center_aligned(PFD_trk_diamond, size[1]/2-55 + Math_rescale_no_lim(0, 0, 20, 169, get_track(PFD_table.Screen_ID) - get_hdg(PFD_table.Screen_ID)), size[2]/2-388, 17, 24, ECAM_GREEN)
end
...
function PFD_draw_hdg_tape(PFD_table)
    ...
    if is_hdg_ok(PFD_table.Screen_ID) == true then
        ...
        sasl.gl.drawTexture(PFD_hdg_tape, size[1]/2-259 - Math_rescale(0, 561, 360, 3609, get_hdg(PFD_table.Screen_ID)), size[2]/2-432, 4096, 110, ECAM_WHITE)
