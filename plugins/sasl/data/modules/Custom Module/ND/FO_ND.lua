...
sasl.registerCommandHandler (ND_Fo_range_cmd_up, 0, function(phase) if phase == SASL_COMMAND_BEGIN then nd_data.config.range = math.min(ND_RANGE_320,nd_data.config.range + 1) end end)
sasl.registerCommandHandler (ND_Fo_range_cmd_dn, 0, function(phase) if phase == SASL_COMMAND_BEGIN then nd_data.config.range = math.max(ND_RANGE_ZOOM_02, nd_data.config.range - 1) end end)
