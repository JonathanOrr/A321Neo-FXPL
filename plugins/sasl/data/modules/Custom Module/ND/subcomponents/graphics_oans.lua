...
local function draw_oans_rwy(data, rwy_start, functions)
    ...
    if semiwidth_px*2 >= 40 then    -- When the runway is sufficiently large, we draw the number on it
        ...
        sasl.gl.drawRotatedText(Font_ECAMfont, m_x , m_y-height/2 , m_x, m_y, m_angle, text_rwy, font_size, true, false, TEXT_ALIGN_CENTER, ECAM_WHITE)
    end
    ...
    sasl.gl.drawRotatedText(Font_ECAMfont, x_shift, y_shift, x_shift, y_shift, m_angle, rwy_start.name, font_size, true, false, TEXT_ALIGN_CENTER, ECAM_WHITE)

    local x_shift = x_end - dist_text * mcos(angle)
    local y_shift = y_end - dist_text * msin(angle)
    sasl.gl.drawRotatedText(Font_ECAMfont, x_shift, y_shift, x_shift, y_shift, 180+m_angle, rwy_start.sibl_name, font_size, true, false, TEXT_ALIGN_CENTER, ECAM_WHITE)
...
local function draw_oans_tower(data, functions, apt, apt_details)
    ...
    if apt_details.tower_pos.lat ~= 0. then -- There's ONE and only ONE tower per airport in XP apt.dat
        ...
        sasl.gl.drawText(Font_ECAMfont,x,y-y_shift, "TWR", height-4, true, false, TEXT_ALIGN_CENTER, {0., 0.6, 0.})
        k_v = height - 5
...
local function draw_oans_mark_taxi(data, functions, apt, apt_details)
    ...
    for i=0,apt_details.routes_len-1 do
        ...
        if route.name_len > 0 then
            ...
            sasl.gl.drawText(Font_ECAMfont,x,y-y_shift, name, height-4, true, false, TEXT_ALIGN_CENTER, COLOR_YELLOW)
        end
    end
end
...
local function draw_oans_mark_gate(data, functions, apt, apt_details)
    ...
    for i=0,apt_details.gates_len-1 do  -- For each gate...
        ...
        sasl.gl.drawRectangle (x-width/2, y-y_shift-3+data.config.range, width, height-4,  {0,0,0})
        sasl.gl.drawText(Font_ECAMfont,x,y-y_shift, name, height-4, true, false, TEXT_ALIGN_CENTER, ECAM_BLUE)
    end
