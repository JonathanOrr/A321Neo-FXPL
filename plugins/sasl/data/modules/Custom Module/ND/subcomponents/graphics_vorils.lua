...
local function draw_upper_info_freq(data, freq, crs, id_text, tuned_id, color)
    -- Frequency
    sasl.gl.drawText(Font_ECAMfont, size[1]-130, size[2]-50, math.floor(freq), 34, true, false, TEXT_ALIGN_LEFT, color)
    sasl.gl.drawText(Font_ECAMfont, size[1]-70, size[2]-50, "." .. leading_zeros_int(Round((freq*100)%100,0), 2), 28, true, false, TEXT_ALIGN_LEFT, color)
    
    -- CRS
    sasl.gl.drawText(Font_ECAMfont, size[1]-165, size[2]-90, "CRS", 34, true, false, TEXT_ALIGN_LEFT, ECAM_WHITE)
    if adirs_is_hdg_ok(data.id) then
        sasl.gl.drawText(Font_ECAMfont, size[1]-15, size[2]-90, crs .. "°", 34, true, false, TEXT_ALIGN_RIGHT, color == ECAM_WHITE and ECAM_BLUE or color)
    else
        sasl.gl.drawText(Font_ECAMfont, size[1]-15, size[2]-90, "---°", 34, true, false, TEXT_ALIGN_RIGHT, ECAM_BLUE)
    end

    -- Identifier
    sasl.gl.drawText(Font_ECAMfont, size[1]-25, size[2]-130, id_text, 34, true, false, TEXT_ALIGN_RIGHT, color)
    ...
    if tuned_symbol ~= "" then
        sasl.gl.drawText(Font_ECAMfont, size[1]-130, size[2]-130, tuned_symbol, 22, true, false, TEXT_ALIGN_RIGHT, ECAM_WHITE)
...
local function draw_upper_right_info_vor(data)
    ...
    sasl.gl.drawText(Font_ECAMfont, size[1]-250, size[2]-50, "VOR" .. curr_nav, 34, true, false, TEXT_ALIGN_LEFT, is_valid and ECAM_WHITE or ECAM_RED)
...
local function draw_upper_right_info_ils(data)
    ...
    sasl.gl.drawText(Font_ECAMfont, size[1]-250, size[2]-50, "ILS" .. curr_nav, 34, true, false, TEXT_ALIGN_LEFT, is_valid and ECAM_WHITE or ECAM_RED)
